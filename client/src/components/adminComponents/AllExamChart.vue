  ...
  <div class="container mx-auto p-4 max-w-4xl bg-base-200 rounded-xl shadow-lg">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 border-b pb-4">
      ...
      <div class="flex gap-2 items-center">
        <!-- Faculty Prev Button -->
        <button @click="prevFaculty" :disabled="currentFacultyIndex === 0" class="btn btn-sm btn-outline btn-primary rounded-lg">
          ⬅️ Previous Faculty
        </button>
        <span class="text-sm font-semibold">{{ currentFacultyIndex + 1 }} / {{ faculties.length }}</span>
        <!-- Faculty Next Button -->
        <button @click="nextFaculty" :disabled="currentFacultyIndex === faculties.length - 1" class="btn btn-sm btn-outline btn-primary rounded-lg">
          Next Faculty ➡️
        </button>
      </div>
    </div>
    ...
    <div v-if="currentFaculty.id" class="space-y-6">
      ...
      <div class="p-4 bg-base-100 rounded-xl shadow-md">
        <h3 class="font-bold text-lg mb-3 text-warning">
          Faculty Progress Overview
        </h3>
        <div class="space-y-3">
          <div class="flex justify-between mb-1 text-sm">
            <span class="font-medium text-warning">
              Completed Departments: {{ facultyProgressForCurrent.completed }}/{{ facultyProgressForCurrent.total }}
            </span>
            <span class="font-bold text-warning text-lg">{{ facultyProgressForCurrent.percent.toFixed(1) }}%</span>
          </div>
          <progress class="progress progress-warning w-full h-2" :value="facultyProgressForCurrent.percent" max="100"></progress>
        </div>
      </div>

      <!-- 2. CURRENT DEPARTMENT VIEW (Focused Card) -->
      <div v-if="currentDepartment" class="p-5 bg-base-100 rounded-xl shadow-xl border-l-4 border-success">
        ...
        <div class="mb-5 border-t pt-3">
          <div class="flex justify-between mb-1 text-sm">
            <span class="font-medium text-success">
              Completed Promotions: {{ getDepartmentProgress(currentDepartment.id).completed }}/{{ getDepartmentProgress(currentDepartment.id).total }}
            </span>
            <span class="font-bold text-success text-xl">
              {{ getDepartmentProgress(currentDepartment.id).percent.toFixed(1) }}%
            </span>
          </div>
          <progress class="progress progress-success w-full h-3" :value="getDepartmentProgress(currentDepartment.id).percent" max="100"></progress>
        </div>

        <!-- Promotions within the Selected Department (Navigation remains internal for detail) -->
        <div v-if="promotionsForDepartment(currentDepartment.id).length" class="space-y-2 ml-4 mt-6 p-3 bg-base-200 rounded-lg shadow-inner">
          ...
          <div class="flex flex-col gap-2 border-b pb-2">
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex gap-2">
                <button @click="prevPromotion(currentDepartment.id)" :disabled="currentPromotionIndex === 0" class="btn btn-xs btn-outline btn-primary btn-square">⬅️</button>
                <span class="text-xs font-medium">{{ currentPromotionIndex + 1 }}/{{ promotionsForDepartment(currentDepartment.id).length }}</span>
                <button @click="nextPromotion(currentDepartment.id)" :disabled="currentPromotionIndex === promotionsForDepartment(currentDepartment.id).length -1" class="btn btn-xs btn-outline btn-primary btn-square">➡️</button>
              </div>
            </div>
            ...
            <div class="w-full">
              <div class="flex justify-between items-center text-xs">
                  <span class="font-medium text-primary">
                    Students Completed: {{ getPromotionProgress(currentPromotion.id).completed }}/{{ getPromotionProgress(currentPromotion.id).total }}
                  </span>
                  <span class="font-bold text-primary text-base">{{ getPromotionProgress(currentPromotion.id).percent.toFixed(1) }}%</span>
              </div>
              <progress class="progress progress-primary w-full h-2" :value="getPromotionProgress(currentPromotion.id).percent" max="100"></progress>
            </div>
        ...
        <div v-else class="text-center text-sm text-gray-500 italic mt-4">No promotions found for this department.</div>

      </div>

      <!-- No Departments Found State -->
      <div v-else-if="departmentsForCurrentFaculty.length > 0" class="text-center mt-10 text-gray-500 italic">
    ...
    <!-- Loading State -->
    <div v-else class="text-center mt-6 text-gray-500 italic text-sm">
    ...
    <div v-if="globalProgress.faculty.total > 0" class="mt-8 pt-4 border-t border-base-300">
      ...
      <div class="flex justify-between mb-1 text-xs">
        <span class="font-medium text-secondary">
          Completed Faculties: {{ globalProgress.faculty.completed }}/{{ globalProgress.faculty.total }}
        </span>
        <span class="font-bold text-secondary">{{ globalProgress.faculty.percent.toFixed(1) }}%</span>
      </div>
      <progress class="progress progress-secondary w-full h-2" :value="globalProgress.faculty.percent" max="100"></progress>
    </div>
  </div>
</template>
...
const departmentsForCurrentFaculty = computed(() => departements.value.filter(d => d.id_faculte === currentFaculty.value.id));
const promotionsForDepartment = (deptId) => promotions.value.filter(p => p.id_departement === deptId);
...
const isPromotionCompleted = (promoId) => {
  ...
  const completed = studentsInPromo.filter(s => resultat.value.some(r => r.id_etudiant === s.id && r.pourcentage_final !== null)).length;
...
onMounted(async () => {
  ...
  globalProgress.value.faculty = { total: totalFaculties, completed: completedFac, percent: totalFaculties===0?0:(completedFac/totalFaculties)*100 };
});
</script>
